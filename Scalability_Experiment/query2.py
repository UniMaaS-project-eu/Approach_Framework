...
/* Must be performed in Torino */
MATCH (pc_last)-[:REQUIRES_PROCESS]->(proc_last:Process)
MATCH (proc_last)-[:PERFORMED_AT]->(site_last:Site {location:'Torino'})

/* NEW: products related to the last PC (inputs and outputs) */
...
OPTIONAL MATCH (res_last)-[:PROVIDED_BY]->(sup_last:Supplier {location:'Torino'})
...
/* 3) For each PC, collect valid Torino sites, resources AND products */
UNWIND pcs AS pc
MATCH (pc)-[:REQUIRES_PROCESS]->(proc:Process)
MATCH (proc)-[:PERFORMED_AT]->(site:Site {location:'Torino'})
OPTIONAL MATCH (pc)-[:USES_RESOURCE]->(res:Resource)

/* NEW: product nodes for each PC */
...
OPTIONAL MATCH (res)-[:PROVIDED_BY]->(s:Supplier {location:'Torino'})
...
/* 7) Build final nodes (NOW INCLUDING IntermediaryProduct nodes) */
WITH
     (
       ...
       /* include intermediary products (if any) for prev & next entries */
